...
public class BookingController {
    ...
    public ResponseEntity<PageDto<Booking>> getAllBookingsForUser(
        ...
        return ResponseEntity.ok(this.bookingAdapter.getAllBookingsForUser(userId, page, size, authHeader));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteBooking(@PathVariable UUID id, @RequestHeader("Authorization") String authHeader){
       this.bookingAdapter.deleteBooking(id, authHeader);
